...
import { api, publicDownloadUrl } from '../lib/api.js';
  ...
  useEffect(() => {
    ...
    api.claim(token)
      .then((payload) => {
        ...
        if (payload.kind !== 'markdown' && payload.ticket) {
          window.location.href = publicDownloadUrl(payload.ticket);
        }
      })
      .catch((err) => {
  ...
  if (payload.kind !== 'markdown') {
    return (
      <Centered>
        ...
        <div className="space-y-1.5 text-center">
          <h1 className="text-lg font-semibold text-ink">{payload.title}</h1>
          <p className="text-sm text-muted">
            {payload.filename} &middot; {formatBytes(payload.size)}
          </p>
        ...
        {payload.ticket && (
          <a
            href={publicDownloadUrl(payload.ticket)}
